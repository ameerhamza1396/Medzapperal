-- Transactional cash-on-delivery checkout.
-- Prices and delivery fees are calculated inside Postgres to prevent tampering.
...
begin
  ...
  loop
    ...
    select coalesce(v_variant.price_override, p.base_price) into v_unit_price
  ...
  loop
    ...
    values(v_order_id,v_variant.id,v_quantity,v_unit_price);
